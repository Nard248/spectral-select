...
Evaluated on the lichen fluorescence dataset across 3,072 configurations, the framework demonstrated that CEA perturbation-based selected wavelengths can \emph{exceed} baseline classification performance while reducing the data size, achieving 95.2\% accuracy with 80 bands versus the 88.2\% baseline with all 192 bands, and maintaining above-baseline accuracy down to 9 bands (95.3\% data reduction).
Robustness analysis confirmed genuine selection intelligence (outperforming the 10,000 random combinations with $12\sigma$ separation). Object-level analysis has shown that challenging specimens benefit most, with performance variance reduced by 54\%.
The 9-band optimal configuration selected one wavelength per excitation source, providing directly actionable sensor design guidelines.
