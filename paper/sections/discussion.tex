...
The framework was evaluated on three datasets with deliberately different characteristics: Lichens (4 morphological classes, 192 bands), Collagen Sponges (3 crosslinker classes, 158 bands), and Drop Data (16 drops, no labels, 214 bands).
Across all three, the method achieved above-baseline performance with substantial data reduction: 95.2\% at $K=80$ on Lichens (+7.0 pp), 85.6\% at $K=30$ on Collagen Sponges (+5.8 pp), and 96.4\% per-pixel KNN-5 accuracy at $K=10$ on Drop Data without any labels in the loop.
The three datasets cover different sample physics (lichen autofluorescence, collagen sponges, droplet chemistry), different acquisition geometries (single-specimen $4{\times}4$ grids vs.\ droplet $4{\times}7$ arrays), and different evaluation regimes (supervised vs.\ unsupervised), yet a single hyperparameter family produces top-tier selections on each.
The dimension-scoring strategy proves to be the only meaningful per-dataset parameter: Lichens prefers PCA-$k{=}3$, Collagen Sponges prefers variance-$k{=}1$, and Drop Data is comparable under either; the rest of the pipeline runs identically.

% ----------------------------------------------------------------------------
\subsection{Comparison to Existing Methods}
\label{sec:discussion_sota}

The framework was compared with eight band-selection methods spanning filter, wrapper, cluster-based, deep, and supervised-embedded families (Section~\ref{sec:results_sota}).
On the Drop Data blind benchmark, the proposed method ties or beats every alternative across $K \in \{3,5,7,10\}$ and wins outright at $K=10$, while supervised Sparse-LASSO (which uses labels) offers no advantage over the unsupervised methods.
This refutes the hypothesis that the framework's gain stems from generic dimensionality reduction or noise suppression: at matched $K$, with the same downstream classifier, other methods do not reach the same accuracy.
The closest competitor across $K$ is Successive Projections (SPA), which is itself a sensible default for fluorescence applications because its orthogonal-projection criterion approximates basis-selection in the EEM space; nonetheless the proposed method overtakes it at higher $K$ and produces a selection with clearer physical interpretation (one band per excitation at the autofluorescence peak rather than four orthogonal projections plus noise).

\subsubsection{Why SAM-Greedy Fails on Drop Data}
SAM-greedy's monotonic degradation with $K$ on Drop Data (Fig.~\ref{fig:drops_knn_vs_k}) is instructive: once an angle-maximizing greedy procedure has selected the informative subspace, additional selections by angular orthogonality are necessarily \emph{low-signal} bands.
On a small-sample, low-class-count dataset like Drop Data, this means the selection ``saturates and then deteriorates,'' a failure mode that any diversity-only selector should be expected to exhibit when the informative signal lives in a low-dimensional subspace.
The MMR component in the proposed framework avoids this by anchoring diversity to a learned \emph{relevance} score, not to spectral angle alone.

% ----------------------------------------------------------------------------
\subsection{Evaluation Metric Choice for Unsupervised Datasets}
\label{sec:discussion_metric_choice}

A subtle methodological finding emerged while building the Drop Data benchmark.
The natural-looking evaluation metric---Adjusted Rand Index (ARI) of Ward clustering on the $K$-band drop-mean vectors against the full-spectrum Ward partition---turns out to be \emph{gameable}: it rewards selections that include one strong band plus arbitrary noise bands.
A method that selects one band at the autofluorescence peak plus four bands with near-zero signal will score perfectly under ARI because Ward clustering in the resulting subspace is dominated by the one informative dimension while the noise dimensions add no within-cluster variance.
The proposed framework's information-rich selection of five correlated bands at different excitations is, paradoxically, penalized by this metric because the redundancy among the five bands tightens within-type clusters without sharpening the across-type boundary in the same way a single-band-plus-noise selection does.

For this reason, the primary metric reported in Section~\ref{sec:results_drops} is per-pixel KNN-5 accuracy with stratified cross-validation: each band must contribute discriminative signal per pixel rather than merely passively occupying a slot.
The methodological note for future band-selection benchmarks is that small-sample, cluster-recovery metrics should be reported alongside per-sample classification metrics; using either alone is insufficient.

% ----------------------------------------------------------------------------
\subsection{Influence Normalization as a Scope Axis}
\label{sec:discussion_normalization}

The choice of normalization for the per-band influence scores (Section~\ref{sec:perturbation}) interacts non-trivially with the discriminative structure of the dataset.
On Lichens and Collagen Sponges, where each class extends over a wide spatial region containing both bright and dim pixels, all three normalization strategies (none, max-per-excitation, variance-normalized) produce comparable top-tier selections.
On Drop Data, where each spectral type is concentrated in a small number of physically distinct drops, max-per-excitation normalization is markedly more reliable: Spearman rank correlation between AE influence and the discriminative ground truth (per-band F-ratio across the three Ward types) flips from negative (variance-normalization) to clearly positive (max-per-excitation), and the mean F-ratio of the selected bands increases by 1.9$\times$ to 3.1$\times$ across preprocessing variants.

Mechanistically, variance-normalization divides each band's influence by its data variance.
On spatially-segregated samples like Drop Data, the discriminative bands are also among the highest-variance bands (because that is where the few bright drops differ from the many dim ones), so the division inverts the desired ranking.
On mixed-media samples like lichens, where within-class variance and between-class variance are not strongly coupled, this inversion does not occur.
The practical guideline is therefore to use \texttt{max\_per\_excitation} for samples where the discriminative signal lives in a small fraction of pixels, and the simpler \texttt{variance} or \texttt{none} normalization for mixed-media samples.
This is a real scope-of-applicability axis that downstream users should be aware of.
