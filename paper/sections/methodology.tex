...
The top $k$ dimensions were then selected for perturbation analysis, where $k$ is treated as a hyperparameter.
Two scoring strategies were compared: pure variance-based ranking and Principal Component Analysis (PCA) decomposition of the latent activations.
Empirical evaluation (Section~\ref{sec:results}) revealed that PCA-based scoring with $k=3$ achieves the best performance on the lichens dataset, while variance-based scoring with $k=1$ provides a strong, parameter-light alternative; the two strategies are reported side by side in the results to expose their trade-off.

Alternative scoring methods include activation magnitude (mean absolute value) and reconstruction sensitivity (MSE when dimension is zeroed); these were tested but consistently underperformed both variance and PCA scoring.
...
Two magnitude levels were swept: a ``medium'' regime $\epsilon \in \{30, 40, 50\}$ and a ``high'' regime $\epsilon \in \{50, 60, 70\}$; results across magnitudes within each regime were aggregated to ensure robustness.
The two regimes produced essentially indistinguishable rankings (within $\pm$2 bands on the top-13 selection), confirming that the method is not sensitive to the exact perturbation scale provided $\epsilon$ is large enough to elicit measurable reconstruction change but small enough that the network remains in its trained operating regime.
...
We use a supervised classification algorithm to quantify how much discriminative power remains in the compressed features; the classification described below serves \emph{only} to evaluate the quality of selected wavelengths---it does not influence the selection process.
