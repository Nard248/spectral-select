...
\IEEEPARstart{H}{yperspectral} imaging has emerged as a transformative technology for remote sensing, material characterization, and biomedical analysis, capturing reflected or emitted light across hundreds of contiguous wavelength bands, exceeding the discriminative capabilities of RGB or multispectral cameras~\cite{plaza2009recent,bioucas2013hyperspectral}.
...
\begin{enumerate}
    \item \textbf{Architectural:} a 3D convolutional autoencoder for 4D ME-HSI with parallel encoder branches per excitation that handle variable emission-band counts after Rayleigh cutoff, and an excitation-averaging operator that captures cross-excitation correlations in a shared latent representation.
    \item \textbf{Methodological:} a perturbation-based wavelength attribution framework that traces latent-dimension importance to individual excitation--emission pairs via finite-difference Jacobians, yielding a causal influence matrix that is then combined with MMR for diverse selection.
    \item \textbf{Empirical:} evaluation on three datasets with distinct properties---Lichens (supervised, 4 classes), Collagen Sponges (supervised, 3 classes, different spectral grid), and Drop Data (entirely unsupervised, 16 droplets, 3 post-hoc spectral types from full-spectrum clustering)---each showing that selected wavelengths exceed or match baseline accuracy at deep reductions.
    \item \textbf{Comparative:} a head-to-head comparison against eight band-selection baselines (variance, PCA-loading, SPA, SAM-greedy, MCUVE, ISSC, BS-Net-FC~\cite{cai2020bsnet}, Sparse-LASSO) on the Drop Data benchmark; the proposed method is top-tier across all $K$ values tested and wins at $K=10$.
    \item \textbf{Methodological note:} on small-sample unsupervised datasets, common cluster-recovery metrics (ARI on per-sample means) systematically reward selections that include noise bands; per-sample classification accuracy is the more rigorous benchmark and is reported as the primary metric throughout.
\end{enumerate}