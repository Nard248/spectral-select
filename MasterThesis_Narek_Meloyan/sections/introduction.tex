...
\begin{enumerate}
    ...
    \item \textbf{Empirical:} evaluation on two ME-HSI fluorescence datasets with distinct sample chemistries---Lichens (supervised, 4 morphological classes, 192 bands) and Collagen Sponges (supervised, 3 crosslinker-concentration classes, 158 bands, different spectral grid)---each showing that selected wavelengths exceed baseline classification accuracy at deep reductions ($\geq$80\% data reduction).
    \item \textbf{Validation:} the learned selection is validated through two complementary, dataset-appropriate procedures: on Lichens via robustness comparison against $10{,}000$ random band combinations (a load-bearing-signal test), and on Collagen Sponges via classifier-family generalization across ten downstream algorithms (KNN, LDA, SVM, MLP, Random Forest, Gradient Boosting) confirming the selected bands are intrinsically informative.
\end{enumerate}