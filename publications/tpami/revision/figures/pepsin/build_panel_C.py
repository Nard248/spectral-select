"""Pepsin KNN accuracy vs K - SOTA comparison plot.

Reads `revision/baselines/results_pepsin_quick/pepsin/method_summary.csv`.
Writes panel_C_knn_vs_K.{png,pdf}.
"""
...
SUMMARY = ROOT / "revision" / "baselines" / "results_pepsin_quick" / "pepsin" / "method_summary.csv"
...
ORDER = ["ae_perturb", "variance", "pca_loading", "spa", "sam_greedy",
         "mcuve", "issc", "bsnet_fc", "sparse_lasso", "random"]
...
def main():
    ...
    ax.set_title(
        "Pepsin-Collagen Dataset: K-band classification accuracy across methods\n"
        "Supervised Sparse-LASSO wins; AE-perturb (ours) is competitive in mid-K range.",
        fontsize=11,
    )
