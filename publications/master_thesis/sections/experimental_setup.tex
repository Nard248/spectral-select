...
To evaluate the framework's generalizability beyond a single specimen type, two ME-HSI fluorescence datasets with distinct sample chemistries were used.
The Lichens dataset, with four expert-annotated morphological classes, served as the primary supervised benchmark.
The Collagen Sponges dataset, with three crosslinker-concentration classes, tested transferability to a chemically distinct fluorophore family.
The two datasets share the broad acquisition geometry (UV-to-near-UV excitation, visible emission, $\sim$10\,nm spectral resolution) but differ in their excitation grid, total valid band count, and number of ground-truth classes, providing two independent tests of the same framework.
...
Ground-truth annotations were generated by manual labeling of nine sponge specimens arranged in a $3 \times 3$ grid, with columns corresponding to the three crosslinker-concentration classes (denoted Class~2, 3, and 4 in the result figures).
For each class, a small interior square ROI in one specimen of the first row provides the labeled training pixels for the downstream classifier; the remaining pixels across all nine specimens form the evaluation set.
This matches the training/test split used in the original IASIM 2026 evaluation.
Figure~\ref{fig:collagen_roi} shows the spatial layout and class assignment.

\begin{figure}[!t]
\centering
\includegraphics[width=0.46\textwidth]{figures/collagen_sponges/CollagenLabels_and_ROI.png}
\caption{Ground-truth annotation and class layout for the Collagen Sponges dataset. Nine sponge specimens are arranged in a $3 \times 3$ grid, with columns corresponding to the three crosslinker-concentration classes. The white interior squares (top row) mark the small ROIs used as supervised training exemplars; the rest of each specimen is the evaluation set. As with Lichens, ROIs were used \emph{only} for post-hoc validation; they did not influence the unsupervised selection pipeline.}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{figures/LichensLabels and ROI.png}
