...
def main(max_configs=None, selection_method="autoencoder"):
    ...
    for i, config in enumerate(tqdm(configs_to_run, desc="Running configurations")):  # Run selected configurations
        ...
        try:
            ...
            # ========================================================================
            # OBJECT-WISE ANALYSIS: Calculate metrics for each spatially separated object
            # ========================================================================
            ...
            print(f"\n  Object-wise metrics for {config_name}:")
            print(f"  {'Object':<10} {'Class':<8} {'Pixels':<10} {'Accuracy':<10}")
            print("  " + "-"*40)
            for obj in object_metrics:
                print(f"  Object-{obj['object_id']:<3} Class-{obj['true_class']:<3} {obj['num_pixels']:<10} {obj['accuracy']:.4f}")

            mean_obj_accuracy = np.mean([obj['accuracy'] for obj in object_metrics])
            std_obj_accuracy = np.std([obj['accuracy'] for obj in object_metrics])
